...
Definition weird' (n : nat) : match n with
                                | 0 => color
                                | 1 => bool 
                                | _ => nat
                              end
...
(* type: even 1 -> False *)
Definition odd1 : odd 1 :=
  fun Hev1 : even 1 =>
    match Hev1 in even n
          return match n with
                   | 1 => False
                   | _ => True
                 end : Prop
...
(* TODO(klao): look up what are these "named arguments" are actually called in coq. *)

Check (@eq_refl nat 5 : @eq nat 5 5).

(* Theorem blue_neq_green : blue <> green. Admitted. *)

Definition blue_neq_green :=
  fun Heqbq : eq blue green =>
    match Heqbq in (eq _ c2)
          return match c2 with
                   | green => even 1
                   | _ => even 0
                 end
    with
      | eq_refl => ev0          (* eq blue blue *)
    end.

Check blue_neq_green.
Print blue_neq_green.
...
(* HW: do the same with 'leb' instead of le_dec. *)
...
Theorem dual_Frobenius_failed (A : Set) p q :
  (forall x : A, q \/ p x) <-> (q \/ forall x : A, p x).
Proof.
  firstorder.
Admitted.

Definition dual_Frobenius := forall (A : Set) p q,
  (forall x : A, q \/ p x) <-> (q \/ forall x : A, p x).

Theorem lem_to_frob : LEM -> dual_Frobenius.
Proof.
...
Theorem frob_to_lem : dual_Frobenius -> LEM.
Proof.
  unfold LEM, dual_Frobenius.
  firstorder.
  Print unit.
  ...
  clear H1.
  cut (p \/ (sig (fun _ : unit => p) -> False)).
    clear H0. clear H.
    intros [Hp | Hw].
    left. assumption.
    right.
    contradict Hw.
    (* { _ : unit | p }  ~=~  exists _ : unit, p *)
    exists tt.
    assumption.
